...
using Tactile.Map;
using Tactile.Menus.Map.ContextSensitive;
...
namespace Tactile
{
    internal partial class Game_Map
    {
        ...
        protected void setup_test_generic(int class_id, int level, Generic_Builds build, int[] growths)
        {
            ...
            if (Global.data_classes.Any(x => x.Value.Tier == 0) && last_added_unit.actor.tier > 0) //Global.game_system.has_tier_0s and actor.tier > 0 //Yeti
                exp_gain += Global.ActorConfig.Tier0LvlCap * Global.ActorConfig.ExpToLvl;
            for (int i = 1; i < 2; i++)
                exp_gain += Global.ActorConfig.LvlCap * Global.ActorConfig.ExpToLvl;
            if (level < 1)
            {
                exp_gain += (level + 1) * (-Global.ActorConfig.ExpToLvl);
                level = 1;
            }
            ...
            int[] wexp = Enumerable.Range(0, Global.weapon_types.Count - 1)
                .Select(x => TactileLibrary.Data_Weapon.WLVL_THRESHOLDS[(int)TactileLibrary.Weapon_Ranks.A])
                .ToArray();

            last_added_unit.actor.setup_generic(
                class_id, level, 0, exp_gain / Global.ActorConfig.ExpToLvl, build,
        ...
        public Map_Object get_map_object(Vector2 loc)
        {
            Destroyable_Object destroyable = get_destroyable(loc);
            if (destroyable != null)
                return destroyable;
            Siege_Engine siegeEngine = get_siege(loc);
            if (siegeEngine != null)
                return siegeEngine;
            LightRune lightRune = get_light_rune(loc);
            if (lightRune != null)
                return lightRune;
 ...
 || Global.scene.scene_type == "Scene_Map_Unit_Editor"
        ...
        internal Maybe<EscapePoint> escape_point_data(int team, int group, Vector2 loc)
        {
            var team_escape_points = EscapePoints.Where(x => team == -1 || x.Team == team);
            // Get escape locs for this team that either have no group set or the passed in group value
            var group_escape_points = team_escape_points
                .Where(x => group == -1 || x.Group == group || x.Group == -1);

            group_escape_points = group_escape_points.Where(x => x.Loc == loc);
            if (!group_escape_points.Any())
                return Maybe<EscapePoint>.Nothing;
            return group_escape_points.First();
        }
        ...
        public bool seize_point_exists(Vector2 loc)
        {
            int team, group;
            return seize_point_exists(loc, out team, out group);
        }
        public bool seize_point_exists(Vector2 loc, out int team, out int group)
        {
            for (int i = 0; i < Seize_Points.Length; i++)
            {
                if (Seize_Points[i] == null)
                    continue;

                foreach (var pair in Seize_Points[i].OrderBy(x => x.Key))
                {
